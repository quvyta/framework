...
5. `.align(Align::Center)` or `Align::End` aligns each line.
6. Text is not selectable with the mouse. Where it is content worth copying, such as a message or an address, add `.selectable(true)` to its node.
...
- **Cutting important text.** Let explanations wrap; cut only lines where the start says enough, like file names.
