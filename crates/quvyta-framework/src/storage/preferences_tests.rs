//! Resolution order, scopes and file handling of the ecosystem's shared preferences.
...
fn resolve(dir: &Path, app: &str) -> Preferences {
    Ecosystem::QUVYTA.preferences_detecting(dir, app, &I18n::builtin(), turkish_kitty)
}
...
#[test]
fn the_ecosystem_id_and_a_missing_key_both_follow_the_shared_file() {
    let dir = folder("follow");
    write(&dir, "quvyta.conf", "language = \"de\"\ntheme = \"amber\"\nicons = \"ascii\"\n");
    write(&dir, "code.conf", "language = \"quvyta\"\ntheme = \"quvyta\"\n");
    let prefs = resolve(&dir, "code");
    assert_eq!(prefs.language(), &text("de", Source::Ecosystem));
    assert_eq!(prefs.theme(), &text("amber", Source::Ecosystem));
    assert_eq!(
        prefs.icons(),
        &Resolved { value: IconMode::Ascii, source: Source::Ecosystem },
        "absent counts as quvyta"
    );
    fs::remove_dir_all(&dir).expect("clean");
}
...
fn a_missing_shared_file_is_created_with_the_detected_values() {
    ...
    assert_eq!(resolve(&dir, "code").language(), &text("tr", Source::Ecosystem), "the next start reads it");
...
fn a_broken_line_falls_back_to_the_detected_value_with_a_located_diagnostic() {
    ...
    write(&dir, "quvyta.conf", "language = \"de\"\ntheme = \nicons = \"sparkly\"\n");
    let prefs = resolve(&dir, "code");
    assert_eq!(prefs.language(), &text("de", Source::Ecosystem), "the good line is kept");
    assert_eq!(prefs.theme(), &text("monochrome", Source::Detected));
    assert_eq!(prefs.icons(), &Resolved { value: IconMode::Nerd, source: Source::Detected });
...
fn the_shared_file_cannot_follow_itself() {
    ...
    assert_eq!(
        ...
        "quvyta.conf:1:1: warning: `theme` cannot follow the ecosystem in the ecosystem's own file; the detected value is used"
    );
    fs::remove_dir_all(&dir).expect("clean");
}

#[test]
fn scope_ecosystem_writes_the_shared_file_and_makes_the_app_follow() {
    let dir = folder("scope-ecosystem");
    ...
    Ecosystem::QUVYTA.set_in(&dir, "code", Shared::Theme, "nordic", Scope::Ecosystem).expect("set");
    assert_eq!(read(&dir, "quvyta.conf"), "language = \"tr\"\ntheme = \"nordic\"\nicons = \"nerd\"\n");
    assert_eq!(read(&dir, "code.conf"), "theme = \"quvyta\"\nengine = \"podman\"\n", "other keys stay");
    assert_eq!(resolve(&dir, "code").theme(), &text("nordic", Source::Ecosystem));
    assert_eq!(resolve(&dir, "focus").theme(), &text("nordic", Source::Ecosystem), "a follower changes too");
...
fn scope_app_writes_only_the_applications_file() {
    ...
    Ecosystem::QUVYTA.set_in(&dir, "code", Shared::Theme, "nordic", Scope::App).expect("set");
    ...
    assert_eq!(resolve(&dir, "focus").theme(), &text("monochrome", Source::Ecosystem));
    fs::remove_dir_all(&dir).expect("clean");
}

#[test]
fn follow_puts_the_application_back_on_the_ecosystem_without_touching_the_shared_file() {
    let dir = folder("follow-back");
    let shared = "# written by hand\nlanguage = \"tr\"\ntheme = \"nordic\"\nicons = \"nerd\"\n";
    write(&dir, "quvyta.conf", shared);
    write(&dir, "code.conf", "theme = \"amber\"\nengine = \"podman\"\n");
    Ecosystem::QUVYTA.follow_in(&dir, "code", Shared::Theme).expect("follow");
...
fn follow_creates_a_missing_file_with_only_that_key() {
    ...
    Ecosystem::QUVYTA.follow_in(&dir, "tools", Shared::Language).expect("follow");
...
fn following_twice_changes_nothing() {
    ...
    ecosystem.follow_in(&dir, "code", Shared::Icons).expect("first");
    let after_first = read(&dir, "code.conf");
    ecosystem.follow_in(&dir, "code", Shared::Icons).expect("second, a key that already follows");
...
fn follow_refuses_a_broken_file_and_leaves_it_as_it_was() {
    ...
    let error = Ecosystem::QUVYTA.follow_in(&dir, "code", Shared::Theme).expect_err("broken");
...
fn two_writers_keep_both_changes() {
    ...
    Ecosystem::QUVYTA.set_in(&dir, "code", Shared::Theme, "nordic", Scope::Ecosystem).expect("code writes");
    Ecosystem::QUVYTA.set_in(&dir, "focus", Shared::Language, "de", Scope::Ecosystem).expect("focus writes");
    Ecosystem::QUVYTA.set_in(&dir, "focus", Shared::Icons, "ascii", Scope::App).expect("focus writes its own");
    ...
    assert_eq!(focus.theme(), &text("nordic", Source::Ecosystem));
    fs::remove_dir_all(&dir).expect("clean");
}
...
fn writers_on_threads_keep_every_change() {
    ...
    let handles: Vec<_> = ["code", "focus", "tools"]
        ...
        .map(|((app, key), value)| {
            let dir = dir.clone();
            std::thread::spawn(move || Ecosystem::QUVYTA.set_in(&dir, app, key, value, Scope::App))
        })
        .collect();
...
#[test]
fn ecosystem_writers_in_the_same_instant_keep_every_change() {
        ...
        .map(|(index, (key, values))| {
            ...
            std::thread::spawn(move || {
                for round in 0..40 {
                    ...
                    Ecosystem::QUVYTA.set_in(&dir, app, key, value, Scope::Ecosystem).expect("set");
                }
            })
        })
        .collect();
...
fn invalid_values_are_refused_without_writing() {
    ...
    for (key, value) in [(Shared::Icons, "sparkly"), (Shared::Theme, "quvyta"), (Shared::Language, " ")] {
        let error = ecosystem.set_in(&dir, "code", key, value, Scope::Ecosystem).expect_err(value);
        assert_eq!(error.kind(), io::ErrorKind::InvalidInput);
    }
    assert!(fs::read_dir(&dir).expect("list").next().is_none(), "nothing written");
    ecosystem.set_in(&dir, "code", Shared::Icons, "NERD", Scope::App).expect("any case");
...
fn a_follow_value_survives_the_applications_self_healing() {
    ...
    let settings = Settings::open(dir.join("code.conf")).member_of(&Ecosystem::QUVYTA).schema(schema).self_heal(true);
...
fn the_files_sit_in_the_xdg_config_folder_on_linux() {
    ...
    let ecosystem_dir = config_root(lookup).map(|base| base.join(Ecosystem::QUVYTA.id())).expect("a folder");
    assert_eq!(ecosystem_dir, root.join("quvyta"));
    resolve(&ecosystem_dir, "code");
    Ecosystem::QUVYTA.set_in(&ecosystem_dir, "code", Shared::Theme, "nordic", Scope::App).expect("set");
...
fn the_runtime_starts_with_the_resolved_values() {
    ...
    let settings = Settings::open(dir.join("code.conf")).member_of(&Ecosystem::QUVYTA);
    let mut env = crate::env::Env::builtin();
    env.apply_settings(&settings);
    assert!(env.diagnostics().is_empty(), "the ecosystem's id is not taken for a theme: {:?}", env.diagnostics());
