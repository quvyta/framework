...
5. Alan yalnızca menüyü göstermek için varsa, örneğin oturumları listeleyen bir durum şeridi öğesi, `.on_left_click(true)` ekle: sol tık menüyü açar, ikinci sol tık kapatır.
...
- **Sol tık da açabilir.** `.on_left_click(true)` ile alan menüsünün butonu gibi çalışır: sol tık menüyü imlecin yanında, sağ tıkın açacağı yerde açar; menü açıkken alana sol tıklamak onu kapatır. Tuşlar ve fare bundan sonra her bağlam menüsündeki gibi çalışır. Basışları kendisi alan bir çocuk, örneğin bir buton ya da bir liste, sol tıkını korur.
