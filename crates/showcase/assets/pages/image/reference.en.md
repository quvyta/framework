...
- `Env::graphics()` is `Graphics::Sixel`: the cells are marked as for kitty, and once the frame is painted the picture is shown only if every cell it may paint (within its clip) still holds its mark; anything over any of it, a blend included, draws the whole picture with half blocks for that frame. Shown, it is shrunk with the box filter to its cells' pixels: a cell is the size the terminal reports for its window divided by its cells, or 10 × 20 where it reports none, and the height is cut down to whole bands of six so the picture never reaches below its last row. Colours go to a fixed palette of 6 × 7 × 6 levels (red, green, blue; 252 entries, only those used are defined). It is written after the cells, inside the frame's synchronized update: the cursor moves to its first cell, then `ESC P 0;1;0 q "1;1;w;h`, the palette, six-pixel bands with `!` repeats, `ST`. It is written again when it is new, moved or cut differently, when a cell under it changed, after a new size and after a program had the terminal; a frame with the same places over unchanged cells writes nothing. When a place goes, the cells under it that the frame did not change are written again, so its pixels do not stay behind. Encodings are kept while their picture is painted.
