...
impl<Msg: 'static> Widget<Msg> for Field<Msg> {
    fn measure(&self, cx: &mut MeasureCx<'_>, available: Size) -> Size {
        ...
        if let Some(column) = self.beside(available.width) {
            ...
            let label_rows = 1 + u16::from(self.required);
            let right = control.height.saturating_add(clamp_u16(i32::try_from(lines.len()).unwrap_or(i32::MAX)));
        ...
        let parts = match self.beside(area.width) {
            Some(column) => {
                ...
                Parts {
                    ...
                    required: self.required.then(|| Rect::new(area.x, area.y + 1, column, 1)),
