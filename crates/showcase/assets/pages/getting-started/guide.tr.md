...
`App`'in altı isteğe bağlı metodu uygulamaya ömrü boyunca eşlik eder. Her birinin bir varsayılanı vardır; uygulama yalnızca gerekenleri yazar. Yukarıdaki Yaşam döngüsü paneli `init`, `resized`, `before_quit` ve `terminating`'i gösterir; showcase'in kendisi onları uygular.
...
- **`preferences(&self, tercihler: &Preferences) -> Option<Msg>`** `Runtime::member(Ecosystem::QUVYTA, "code")` ile başlatılan bir uygulamanın ekosistemle paylaştığı dili, temayı ve ikonları duyar: açılışta grafiğin hemen ardından, uygulama çalışırken başka bir uygulama `quvyta.conf`'u ya da bu uygulamanın kendi dosyasını her değiştirdiğinde yeniden. Çalışma motoru ekranı zaten değiştirmiştir; açık bir ayar ekranı yeni değerleri burada `Appearance::refresh` ile alır. Zaten söylediğini yeniden yazan bir dosya bildirilmez, bu yüzden uygulamanın kendi kaydettiği değişiklik asla döngüye girmez.
...
Yalnızca bir şey bildiren kancalar (`resized`, `graphics`, `preferences`, `before_quit`, `terminating`, `action`, `clipboard`) durumu okur ve bir mesajla cevap verir; iş başlatan kanca (`init`) `update` gibi bir komut döndürür. Test düzeneği hepsini terminalin çalıştırdığı yerde çalıştırır: `Harness::new(app, 120, 30)` 120 × 30'u ve yarım bloğu bildirir, `init`'i çalıştırır, `resize(60, 20)` 60 × 20'yi bildirir, `set_graphics(Graphics::Kitty)` kitty'yi bildirir, `Harness::member_in(app, Ecosystem::QUVYTA, &klasör, "code", 120, 30)` bir üyeyi testin kendi klasöründe başlatır ve `poll_preferences()` dosyalarını yeniden okur, `press("ctrl+q")` `before_quit`'e sorar, `terminate(Termination::Hangup)` `terminating`'e haber verir.
