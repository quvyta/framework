...
- **Haftayı bölge, bölge yoksa dil belirler.** Ay ve gün adları `quvyta.date` anahtarlarından gelir. Haftanın ilk günü `I18n::first_weekday()`'den gelir: sistem bir ülke söylüyorsa (`LANG=en_GB.UTF-8`) o ülkenin Unicode CLDR'deki günüdür, Birleşik Krallık'ta pazartesi, ABD'de pazar; söylemiyorsa dilin `quvyta.date.first-weekday` anahtarıdır, İngilizcede pazar, Türkçede pazartesi, bu anahtarı vermeyen bir dilde pazartesi. Uygulaman haftaları nerede sayıyorsa aynı metodu çağır; böylece haftalık hedef ile takvim aynı günden başlar. `Env` olmayan `update` içinde `qframe::i18n::first_weekday()` aynı cevabı verir.
