...
- `.line_pending()` — `true` when the person has typed something since the last key that ends a line: `write`, the widget's keys and a person's paste into the widget count, `paste` and mouse reports do not. Enter (`\r`, `\n`) ends the line, Ctrl+C and Ctrl+U throw it away; text and Tab start one; escape sequences (arrows, function keys, Alt chords), Esc, Backspace, Ctrl+W and Ctrl+D leave it as it was. `false` for a new session.
...
- Every key reaches the program, however fast it follows the one before: the terminal takes text, so Enter and Space are never guessed to be a held key, and repeats the terminal reports are sent too.
- `CSI row;col f` (HVP) is drawn as `CSI row;col H` (CUP), which it means; a sequence with a private marker, a sub-parameter or an intermediate byte before its `f` is left alone. The DEC line drawing set is followed in G0 and G1, with Shift Out and Shift In: while it is in use, the bytes `_` to `~` show as the characters they stand for (`lqqkx` as `┌──┐│`); `ESC ( B` and a full reset (`ESC c`) return to ASCII. Both are followed across reads. Autowrap mode (`CSI ?7 l`) is not: a line that reaches the last column always wraps.
