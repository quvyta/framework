//! The ecosystem's one switch for the update notice: whether its applications ask once a day if a
//! newer version of themselves is out.
//!
//! It lives in the ecosystem's shared file only, as `update-notice = false`, because it is a choice
//! about the ecosystem rather than one application: a person who does not want to be told turns it
//! off once. A missing key is on, so an ecosystem that never heard of the notice has it.

use std::fs;
use std::io;
use std::path::Path;

use super::{Ecosystem, SettingValue, Settings};
...
impl Ecosystem {
    /// Whether the ecosystem's applications say when a newer version is out: the shared file's
    ...
    /// [`update_notice`](Self::update_notice) with `config_dir` as the ecosystem's folder instead of
    /// this platform's, for a test or a demo.
    #[must_use]
    pub fn update_notice_in(&self, config_dir: &Path) -> bool {
        let path = config_dir.join(super::ecosystem::file_name(self.id()));
        !path.exists() || from_shared(&Settings::open(path))
    }

    /// Turns the update notice on or off for every application of the ecosystem, in the shared file.
    ...
    /// [`set_update_notice`](Self::set_update_notice) with `config_dir` as the ecosystem's folder
    ...
    pub fn set_update_notice_in(&self, config_dir: &Path, on: bool) -> io::Result<()> {
        ...
        let shared = config_dir.join(super::ecosystem::file_name(self.id()));
...
mod tests {
    ...
    #[test]
    fn the_notice_is_on_until_the_ecosystem_turns_it_off_and_other_keys_stay() {
        let dir = scratch("switch");
        let ecosystem = Ecosystem::QUVYTA;
        assert!(ecosystem.update_notice_in(&dir), "an ecosystem that never chose has it");
        std::fs::create_dir_all(&dir).expect("folder");
        std::fs::write(dir.join("quvyta.conf"), "theme = \"amber\"\n").expect("shared file");
        assert!(ecosystem.update_notice_in(&dir), "a shared file without the key has it");
        ecosystem.set_update_notice_in(&dir, false).expect("saved");
        assert!(!ecosystem.update_notice_in(&dir));
        assert!(!ecosystem.preferences_in(&dir, "code", &I18n::builtin()).update_notice(), "preferences read it too");
        ...
        ecosystem.set_update_notice_in(&dir, true).expect("saved");
        assert!(ecosystem.preferences_in(&dir, "code", &I18n::builtin()).update_notice());
