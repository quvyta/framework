...
- `Resize { edge, dx, dy }` — kenar ya da köşe o kadar hücre hareket eder; üst ve alt kenarda `dx`, sol ve sağ kenarda `dy` sıfırdır.
...
- Başlık: sürüklemek taşır, çift tık büyütür ya da eski boyuta döner.
- İşaretler: küçült, büyüt ya da eski boyut, kapat; her birinin üç hücresi birlikte aydınlanır.
- Gövde: sağ sütun, alt satır ve köşesi boyutlandırır; gövdenin kalanı içindekine aittir.
- `alt` ile sol tuş: her yerden taşır. `alt` ile sağ tuş: en yakın kenardan ya da köşeden boyutlandırır.
...
- `split-handle` — kenar tutamakları; `hover`, sürüklenirken `active`.
