...
- `NodeMut::on_clipboard(ClipboardKey::Cut | Copy | Paste, mesaj)` — odak düğümde ya da içindeyken `ctrl+x`, `copy`'nin tuşları ya da `paste`'in tuşları `mesaj`'ı gönderir; metin olmayan şeylerin listesi, örneğin bir dosya yöneticisinin satırları için. Fareyle seçilmiş metin varken üç tuş da onundur, odaktaki bileşen de tuşu önce görür, bu yüzden metin alanı kendi metnini kopyalamaya ve kesmeye devam eder. Üstlenen en içteki düğüm kazanır. Her tuş için bir kez çağrılır.
