...
- **Broken files never stop the application.** Syntax errors and unusable entries become diagnostics; everything readable is still used. Before a broken file is first overwritten, it is copied next to itself with `.bak` added to its name: `settings.toml.bak`, or `code.conf.bak` for an ecosystem file.
...
Settings are one file; an application has others. Five things belong to all of them, and they live here so every application in the ecosystem does them the same way.

1. **Four folders, not one.** `config_dir("qfocus")` is for settings, `data_dir("qfocus")` for the application's own records. A recorded session is not a setting, and on Linux it does not belong in `.config`. `state_dir("qfocus")` is for what it remembers between runs, such as the result of its last background check, and `cache_dir("qfocus")` for anything it can rebuild, so a user can back up the one and clear the other. All four give `None` when the platform has no home to write in, and none creates the folder: `fs::create_dir_all` before the first write.
...
4. **Wake a service when the last instance closes, with `InstanceLock`.** Every instance holds `InstanceLock::shared(path)` for as long as it runs; any number can at once. A background service that cleans up after them calls `InstanceLock::wait_exclusive(path)` on a thread of its own: it sleeps in the kernel and returns when the last shared lock is released, whether that instance quit, crashed or was killed. `try_exclusive(path)` asks the same question without waiting. Hold the exclusive lock only while cleaning up: an instance that starts meanwhile waits in `shared` until the cleanup is over, instead of running into it. Keep this lock on its own file, apart from an `AppLock`.
5. **One file per machine with `machine_name()`.** When the data folder is synced between machines, a file both of them write is a file one of them overwrites. Put the machine in the name instead: `format!("running-{machine}.toml")`. The name is `uname -n` on Unix and `COMPUTERNAME` on Windows, and it comes back ready for a file name: surrounding whitespace removed, any character other than a letter, a digit, `-`, `_` or `.` turned into `-`, dots at either end dropped. `None` means the system gave nothing usable; pick a fallback name of your own then.

## An ecosystem of applications

Applications made to be used together keep their settings in one folder, so the user finds all of them in one place. An `Ecosystem` names that folder; `Ecosystem::QUVYTA` is the Quvyta ecosystem.

1. **Load from the ecosystem's folder.** `Settings::load_member(&Ecosystem::QUVYTA, "code")` reads `~/.config/quvyta/code.conf` on Linux: the same TOML, the same schema and healing, only the file name says which application it belongs to. `ecosystem.shared_file()` is `quvyta.conf`, the settings every member shares, and `ecosystem.app_dir("code")` is the folder beside the file for everything else the application configures. Its state and cache go to `ecosystem.state_dir("code")` and `ecosystem.cache_dir("code")`, `~/.local/state/quvyta/code` and `~/.cache/quvyta/code` on Linux. On macOS the folder is `~/Library/Application Support/Quvyta`, on Windows `%APPDATA%\Quvyta`: there users read folder names as names, so the title is used. File names are always lowercase.
2. **Move the old settings in once, at start.** An application that kept its settings in a folder of its own calls `Ecosystem::QUVYTA.adopt("packages", &old)` before loading, with `old` the folder it used, for example `config_dir("quvyta-packages")`. `settings.toml` becomes `packages.conf`, every other file keeps its place under `packages/`, and emptied old folders are removed. When the old folder already is the application's folder, as `~/.config/quvyta/focus` is for focus, only `settings.toml` moves.
...
- **Resolving `XDG_STATE_HOME` or `XDG_CACHE_HOME` yourself.** A relative value must be ignored and an empty one counts as unset; `state_dir` and `cache_dir` already follow those rules.
...
- **Letting the lock be dropped.** `AppLock::acquire(path)?;` takes the lock and releases it on the same line. Keep the value for as long as the instance runs; the same goes for `InstanceLock::shared`.
- **Counting instances in a file.** A counter a crashed instance never decreased keeps a service waiting forever. The shared lock needs no counting: the kernel lets go of a dead process's lock.
- **Waiting for the last instance in a loop.** `wait_exclusive` returns at once when nobody runs, so calling it again straight after spins. Wait once, clean up, and let the next instance start the service again.
