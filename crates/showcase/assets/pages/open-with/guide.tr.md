...
- **Açmak doğru yoldan gider.** Terminal programına (`Terminal=true`) terminal devredilir, program bitince ekran geri gelir. Grafik program uygulamanın yanında başlar. `DISPLAY` ya da `WAYLAND_DISPLAY` yoksa grafik program açılamaz; `launch` denemek yerine bunu söyler.
