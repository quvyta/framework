...
- `.id(ad)` — hangi görünüm çiziyorsa satırlara ad verir; böylece `Command::focus(ad)` klavyeyi onlara verir. Klavye satırlardaysa görünüm değişince de orada kalır.
- `.show(ui)` — yöneticiyi ekler ve onu tutan sütunu döner; `.fill()` ya da bir boyut için. Sütun odak almaz; satırlara yöneticinin `.id(ad)` metoduyla ad ver. Ad soran diyalog açıkken o da eklenir; bir katmandır ve yer tutmaz.
...
- Seçili bir girdiden başlayan sürükleme bütün seçimi, başka bir girdiden başlayan yalnızca o girdiyi taşır. İmlecin altındaki klasör onları alabildiği sürece vurgu tonunu alır. Bir klasörün üstünde bırakılınca içine taşınırlar; bırakırken `ctrl` basılıysa her kopyalama gibi arka planda bir iş olarak kopyalanırlar. Ctrl'yi fare olayında bildirmeyen terminal taşır. Bir klasör hiçbir zaman kendini ya da kendi içindeki bir klasörü almaz. Listede ve simgelerde gösterilen klasörün kendi satırı yukarı çıkmanın yoludur: oraya bırakılan bir üstteki klasöre gider, kökteyken hiçbir şey almaz. Bir dosyanın, sürüklenenlerden birinin üstünde ya da düz görünümde satırların dışında bırakılırsa hiçbir şey olmaz; ağaçta satırların altındaki boş alan köktür. Klasörde zaten olan bir ad sebebiyle reddedilir, hiçbir şeyin üzerine yazılmaz.
...
- `shift+up` `shift+down` (simgelerde `shift+left` `shift+right`), `shift+pgup` `shift+pgdn`, `shift+home` `shift+end` seçimi başladığı yerden uzatır; `ctrl+a` gösterilen her girdiyi seçer, kökü ve gösterilen klasörün kendi satırını hiçbir zaman; `esc` yalnızca imlecin girdisini seçili bırakır, tek girdi seçiliyken ya da hiç seçim yokken yöneticiyi tutana geçer, böylece bir pencere yine onunla kapanır.
- `ctrl+x` seçimi keser, `ctrl+c` kopyalar; seçim yoksa imlecin altındaki girdiyi. Kök ve düz görünümde gösterilen klasörün kendi satırı hiçbir zaman taşınmaz. `ctrl+v` bekleyeni yapıştırır: listede ve simgelerde gösterilen klasöre, ağaçta imlecin üstünde durduğu klasöre, imlecin altındaki dosyanın klasörüne, imleç yoksa köke. Üzerinde iş yapacağı bir şey olmayan tuş kendi hâline bırakılır. Tuşlar yalnızca satırlar odaktayken ve fareyle seçilmiş bir metin yokken satırlarındır; `ctrl+c` ve `ctrl+v` tuş haritasındaki `copy` ve `paste`'i izler. Zaten alınmış bir ad reddedilir ve söylenir; hiçbir şeyin üzerine yazılmaz.
