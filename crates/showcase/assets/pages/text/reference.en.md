...
- The cut mark is `text::ELLIPSIS` (`…`). In ASCII glyph mode `PaintCx::text` draws it as `text::ASCII_ELLIPSIS` (`~`), in the same single cell, so every widget cuts with an ASCII mark there and a cut text is equally wide in every mode. A `…` in the application's own text is drawn as `~` in ASCII mode as well.
