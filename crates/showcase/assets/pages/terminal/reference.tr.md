...
- `.line_pending()` — kişi satırı bitiren son tuştan beri bir şey yazdıysa `true`: `write`, bileşenin tuşları ve kişinin bileşene yapıştırması sayılır, `paste` ve fare bildirimleri sayılmaz. Enter (`\r`, `\n`) satırı bitirir, Ctrl+C ve Ctrl+U satırı atar; metin ve Tab bir satır başlatır; kaçış dizileri (oklar, işlev tuşları, Alt birleşimleri), Esc, Backspace, Ctrl+W ve Ctrl+D onu olduğu gibi bırakır. Yeni oturumda `false`.
