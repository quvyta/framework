...
- Order per key: the application's value (anything but the ecosystem's id), the shared file, detection (the language by `I18n::detect`, the theme always `monochrome`, the icons by `detect_glyph_mode`).
- A key missing from the application's file follows the ecosystem.
